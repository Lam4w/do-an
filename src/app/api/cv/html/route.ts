...
export async function GET(req: Request) {
  try {
    ...
    if (snapshot?.contentMain) {
      ...
      res = `
      <div class='container'>
        <div class='${snapshot.settings.template.toLowerCase()} ${snapshot.settings.layout} ${snapshot.settings.fontSize} ${snapshot.settings.spacing} ${snapshot.settings.titleAlignment}'>
          ${content}
        </div>
      </div>`;
    }
