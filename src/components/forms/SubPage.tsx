...
import { SubPageSchema } from '@/lib/types'
import {
  deleteSubPage,
  getPages,
  upsertSubPage,
...
const CreateFunnelPage: React.FC<CreateFunnelPageProps> = ({
  ...
  //ch
  const form = useForm<z.infer<typeof SubPageSchema>>({
    resolver: zodResolver(SubPageSchema),
    mode: 'onChange',
    defaultValues: {
      name: '',
      pathName: '',
  ...
  const onSubmit = async (values: z.infer<typeof SubPageSchema>) => {
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      type="button"
                      onClick={async () => {
                        const response = await deleteSubPage(defaultData.id)
                        router.refresh()
                      }}
                    >
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      type="button"
                      onClick={async () => {
                        const response = await getPages(ownerId)
                        const lastFunnelPage = response.find(
                          (funnel) => funnel.id === pageId
                        )?.subPages.length
